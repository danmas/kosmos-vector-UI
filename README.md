...
- ✅ **Knowledge Graph Navigation v2.3.2**: Интерактивная фильтрация узлов через Alt+клик (показать только узел и его связи) и Ctrl+клик (добавить связанные узлы к фильтру)
...
*   **Knowledge Graph**: Интерактивный граф зависимостей (D3.js) с продвинутой фильтрацией:
  - **Alt+клик** на узел — показывает только этот узел и его связи (сброс всех фильтров)
  - **Ctrl+клик** на узел — добавляет связанные узлы к текущему фильтру (аддитивно)
  - **Двойной клик** — фокус на один узел
...
*   **AI Integration**: 
  ...
  - Google GenAI SDK (`gemini-3-flash-preview`) - клиентская и серверная стороны для Logic Architect
...
Logic Architect поддерживает **два режима** анализа:

| Режим | Кнопка | Описание |
|-------|--------|----------|
| **Client** | `Build (Client)` | Прямой вызов Google Gemini API из браузера. Требует `VITE_GEMINI_API_KEY` |
| **Server** | `Build (Server)` | Вызов через сервер 3200 (`POST /api/items/{id}/analyze-logic`). Требует `API_KEY` на сервере |
...
- **Автосохранение**: Результат автоматически сохраняется на сервере после успешного анализа
...
6. Выберите режим анализа:
   - **Build (Client)** — использует API ключ из браузера
   - **Build (Server)** — использует API ключ на сервере (требует `item.id`)
...
- Переменная окружения `API_KEY` на сервере 3200
- AiItem должен существовать в базе данных
...
- `GET /api/items/:id/logic-graph` - получение/сохранение анализа логики функции
