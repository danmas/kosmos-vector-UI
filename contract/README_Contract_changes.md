...
**Клиентская фильтрация (v2.1.1):**
- Маски применяются **локально на клиенте** с помощью `minimatch` для мгновенного отклика
- При изменении `includeMask` в UI — файлы фильтруются без перезагрузки дерева с сервера
- Ручной выбор файлов сохраняется в `fileSelection` и восстанавливается после перезагрузки страницы
- Индикатор режима: **🎯 По маске** или **✋ Ручной выбор**
- **Игнорируемые папки схлопнуты**: Папки, попадающие в `ignorePatterns` или базовые игноры (`.cursor`, `.git`, `.vscode`, `node_modules` и др.), отображаются свернутыми (collapsed) по умолчанию
- **Умные игноры по умолчанию**: Дефолтные `ignorePatterns` включают `**/.cursor/**`, `**/.git/**`, `**/.vscode/**`, `**/node_modules/**`, `**/venv/**`, `**/tests/**` и другие служебные папки
...
## 9. Маршрут получения списка context codes (версия 2.3.1)

**Дата изменения:** 02 января 2026

**Новый маршрут:** `GET /api/contexts`

**Описание:** Возвращает список всех доступных context codes из конфигураций pipeline. Используется для динамического построения списка выбора контекста в UI.

**Ответ:**
```json
{
  "success": true,
  "contexts": ["CARL", "PROJECT_A", "TEST"]
}
```

**Источник данных:** Ключи объекта из `pipeline-configs.json`

**Изменения на фронтенде:**
- `DataCacheContext` загружает список context codes при старте через `useEffect`
- Sidebar отображает динамический список вместо hardcoded значений

---

## 10. Формат labels шагов pipeline (версия 2.3.1)
