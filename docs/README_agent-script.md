...
### Обработка ошибок (500 Error)

При ошибках генерации или выполнения скрипта сервер возвращает расширенный JSON:

```json
{
  "success": false,
  "error": "ReferenceError: rows is not defined",
  "human": "Ошибка выполнения: переменная rows не определена в скрипте.",
...
3.  **Визуализация результата:**
    ...
    *   **Agent Script**: Просмотр сгенерированного JS-кода с **подсветкой синтаксиса**. При ошибках выполнения скрипт также доступен для отладки.
     *   **Raw Data**: Просмотр сырых данных в формате JSON.
     *   **Copy to Clipboard**: Быстрое копирование кода скрипта во вкладке Agent Script.
 4.  **Интеграция с графом и фильтрацией (v2.2.2):**
     *   **Apply to Filter**: Генерирует оптимизированное регулярное выражение (до 2000 символов) для мгновенного нахождения всех целевых узлов.
     *   **Smart Sync**: Фильтр, примененный в Инспекторе через Natural Query, автоматически подхватывается Графом (Knowledge Graph), даже если Инспектор не активен.
     *   **Decoupled Logic**: Пользователь может расширять отфильтрованный граф вручную (Ctrl+LMB) или использовать локальный поиск в Графе, который имеет приоритет над общим фильтром.
